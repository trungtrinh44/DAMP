...
class ResNet(nn.Module):
    ...
    def classify(self, x, train):
        x = nn.Dense(
            self.num_classes,
            dtype=self.dtype,
            kernel_init=dense_layer_init_fn,
            name="classify",
        )(x)
        return x
    ...
    def __call__(self, x, train):
        x, features = self.encode(x, train)
        logits = self.classify(x, train)
...
PreActResNet18 = partial(
    ResNet, stage_sizes=[2, 2, 2, 2], block_cls=ResNetBlockV2, resnet_v2=True
)
PreActResNet34 = partial(
    ResNet, stage_sizes=[3, 4, 6, 3], block_cls=ResNetBlockV2, resnet_v2=True
)
