...
def get_data_loader(
    ...
    transform = torchvision.transforms.Compose(
        [
            *(
                ...
                if dataset
                in ("cifar10", "cifar100", "svhn", "vgg_cifar10", "vgg_cifar100")
                else []
            ),
            torchvision.transforms.Normalize(
                *(NORM_STAT[dataset] if norm_stat is None else norm_stat)
            ),
        ]
    )
    train_data = train_data_cls(
        transform=torchvision.transforms.Compose([*augment_transform, transform])
    )
    if train_only:
    ...
    if validation:
        ...
        train_idx, valid_idx = train_test_split(
            np.arange(len(train_data.targets)),
...
class CorruptDataset(torch.utils.data.Dataset):
    def __init__(self, root, corrupt_types, intensity, transform=None):
        self.data = np.concatenate(
            [
                np.load(os.path.join(root, f"{corrupt_type}.npy"))[
                    intensity * 10000 : (intensity + 1) * 10000
                ]
        ...
        self.label = np.concatenate(
            [
                np.load(os.path.join(root, "labels.npy"))[
                    intensity * 10000 : (intensity + 1) * 10000
                ]
            ]
            * len(corrupt_types),
...
def get_corrupt_data_loader(
    dataset, intensity, batch_size=64, root_dir="data/", num_workers=4, norm_stat=None
):
    corrupt_type = [
        "saturate",
        "shot_noise",
    ...
    if dataset == "cifar10":
        test_data = CorruptDataset(
            os.path.join(root_dir, "CIFAR-10-C"), corrupt_type, intensity, transform
        )
        test_loader = DataLoader(
            test_data,
    ...
    if dataset == "cifar100":
        test_data = CorruptDataset(
            os.path.join(root_dir, "CIFAR-100-C"), corrupt_type, intensity, transform
        )
        test_loader = DataLoader(
            test_data,
            batch_size=batch_size,
