...
def yxyx_to_cycxhw(boxes):
    ...
    if boxes.shape[-1] != 4:
        raise ValueError(
            "Last dimension of boxes must be 4 but is {:d}".format(boxes.shape[-1])
        )
...
def cycxhw_to_yxyx(boxes):
    ...
    new_boxes = tf.stack([boxes_ymin, boxes_xmin, boxes_ymax, boxes_xmax], axis=-1)
    return new_boxes
...
def horizontal_flip_boxes(normalized_boxes):
    ...
    if normalized_boxes.shape[-1] != 4:
        raise ValueError(
            "boxes.shape[-1] is {:d}, but must be 4.".format(normalized_boxes.shape[-1])
        )
...
def decode_boxes(encoded_boxes, anchors, weights=None):
    ...
    with tf.name_scope("decode_boxes"):
        ...
        anchor_ymin, anchor_xmin, anchor_ymax, anchor_xmax = tf.split(anchors, 4, -1)
        anchor_h = anchor_ymax - anchor_ymin
        anchor_w = anchor_xmax - anchor_xmin
        anchor_yc = anchor_ymin + 0.5 * anchor_h
        anchor_xc = anchor_xmin + 0.5 * anchor_w
...
def filter_boxes(boxes, scores, image_shape, min_size_threshold):
    ...
    with tf.name_scope("filter_boxes"):
        ...
        min_size = tf.cast(tf.math.maximum(min_size_threshold, 0.0), dtype=boxes.dtype)
    ...
    if batch_size == 1:
        ...
        if aux_instances:
            selected_aux_instances = [
                tf.squeeze(tf.gather(a, selected_indices, axis=1), axis=1)
        ...
        batch_indices = tf.expand_dims(tf.range(indices_shape[0]), axis=-1) * tf.ones(
            [1, indices_shape[-1]], dtype=tf.int32
        )
        gather_nd_indices = tf.stack([batch_indices, selected_indices], axis=-1)
...
def bbox_overlap(boxes, gt_boxes):
    ...
    with tf.name_scope("bbox_overlap"):
        ...
        boxes_invalid_mask = tf.less(tf.reduce_max(boxes, axis=-1, keepdims=True), 0.0)
...
def bbox_generalized_overlap(boxes, gt_boxes):
    ...
    with tf.name_scope("bbox_generalized_overlap"):
        assert boxes.shape.as_list()[-1] == 4, "Boxes must be defined by 4 coordinates."
        assert (
            gt_boxes.shape.as_list()[-1] == 4
...
def bbox_intersection_over_area(boxes, gt_boxes):
    ...
    with tf.name_scope("bbox_overlap"):
        ...
        ioa = tf.where(padding_mask, -1.0, ioa)

        # Fills -1 for invalid (-1) boxes.
        boxes_invalid_mask = tf.less(tf.reduce_max(boxes, axis=-1, keepdims=True), 0.0)
        ioa = tf.where(boxes_invalid_mask, -1.0, ioa)
...
def box_matching(boxes, gt_boxes, gt_classes):
    ...
    matched_gt_classes = tf.where(
        background_box_mask, tf.zeros_like(matched_gt_classes), matched_gt_classes
    )

    matched_gt_indices = tf.where(
        background_box_mask, -tf.ones_like(argmax_iou_indices), argmax_iou_indices
    )
...
def bbox2mask(
    bbox: tf.Tensor, *, image_height: int, image_width: int, dtype: tf.DType = tf.bool
) -> tf.Tensor:
