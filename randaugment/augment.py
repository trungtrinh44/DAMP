...
def translate(
    ...
    """
    transforms = _convert_translation_to_transform(
        translations
...
def cutout(image: tf.Tensor, pad_size: int, replace: int = 0) -> tf.Tensor:
    ...
    image = _fill_rectangle(
        image, cutout_center_width, cutout_center_height, pad_size, pad_size, replace
    )
...
def cutout_video(
    ...
    upper_pad = tf.maximum(0, video_height - cutout_center_height - pad_size_height)
    ...
    forward_pad = tf.maximum(0, video_depth - cutout_center_depth - pad_size_depth)
...
def gaussian_noise(image: tf.Tensor, low: float = 0.1, high: float = 2.0) -> tf.Tensor:
...
def sharpness(image: tf.Tensor, factor: float) -> tf.Tensor:
    ...
    if orig_image.shape.rank == 3:
        ...
        degenerates = [
            tf.nn.conv3d(
                channel, kernel, strides, padding="VALID", dilations=[1, 1, 1, 1, 1]
            )
...
def unwrap(image: tf.Tensor, replace: int) -> tf.Tensor:
    ...
    image = tf.slice(
        image, [0] * image.shape.rank, tf.concat([image_shape[:-1], [3]], -1)
    )
    return image
...
def _apply_func_with_prob(
    func: Any, image: tf.Tensor, bboxes: Optional[tf.Tensor], args: Any, prob: float
):
    ...
    def _make_tf_policies(self):
        ...
        for policy in self.policies:
            ...
            for policy_info in policy:
                ...
                assert_ranges.append(tf.Assert(tf.less_equal(prob, 1.0), [prob]))
...
class RandAugment(ImageAugment):
    ...
    def _distort_common(
        self, image: tf.Tensor, bboxes: Optional[tf.Tensor] = None
    ) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
...
class MixupAndCutmix:
    ...
    def distort(
        ...
        augment_cond = tf.less(
            tf.random.uniform(shape=[], minval=0.0, maxval=1.0), self.mix_prob
        )
        # pylint: disable=g-long-lambda
