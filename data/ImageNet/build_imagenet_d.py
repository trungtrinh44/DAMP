...
tf.app.flags.DEFINE_string("train_directory", None, "Training data directory")

tf.app.flags.DEFINE_string("validation_directory", None, "Validation data directory")
...
tf.app.flags.DEFINE_integer(
    "validation_shards", 1, "Number of shards in validation TFRecord files."
)

tf.app.flags.DEFINE_integer(
    "num_threads", 1, "Number of threads to preprocess the images."
)
...
# tf.app.flags.DEFINE_string("labels_file", "imagenet_a_synsets.txt", "Labels file")
...
def _find_image_files(data_dir):
    ...
    with open("imgnet_d2imgnet_id.json") as inp:
        label2id = json.load(inp)
    ...
    for synset in label2id.keys():
        ...
        matching_files.extend(tf.gfile.Glob(jpeg_file_path))
        labels.extend([label2id[synset]] * len(matching_files))
    ...
    print(
        ...
        % (len(filenames), len(label2id), data_dir)
    )
...
def _process_dataset(name, directory, num_shards, synset_to_human):
    ...
    _process_image_files(name, filenames, synsets, labels, synsets, num_shards)
...
def main(unused_argv):
    ...
    if FLAGS.train_directory != None:
        _process_dataset(
            "train", FLAGS.train_directory, FLAGS.train_shards, synset_to_human
        )
